...
const LEARNED_KEY = "yomiko-verbs-learned";
...
function Verbs() {
  ...
  const [selectedGroup, setSelectedGroup] =
    useState<(typeof groups)[number]>("All");

  const [search, setSearch] = useState("");

  const [selectedVerb, setSelectedVerb] =
    useState<VerbItem | null>(null);
  ...
  const learnedCount = learned.filter((id) =>
    verbsData.some((item) => item.id === id),
  ).length;

  const totalVerbs = verbsData.length;

  const remainingCount = Math.max(
    totalVerbs - learnedCount,
    0,
  );
  ...
  const progress =
    totalVerbs > 0
      ? Math.round(
          (learnedCount / totalVerbs) * 100,
        )
      : 0;

  const levelCount = (level: string) => {
    if (level === "All") {
      return verbsData.length;
    }

    return verbsData.filter(
  ...
  const filteredTotal = verbsData.filter((item) => {
    const levelMatch =
      selectedLevel === "All" ||
      item.level === selectedLevel;
  ...
  const filteredLearned = learned.filter((id) =>
    verbsData.some((item) => {
      const levelMatch =
        selectedLevel === "All" ||
        item.level === selectedLevel;
  ...
  const filteredProgress =
    filteredTotal > 0
      ? Math.round(
          (filteredLearned / filteredTotal) * 100,
        )
      : 0;
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl dark:text-white">
                Japanese Verb Library
              </h1>

              <p className="mt-1 text-xs text-slate-500 sm:text-sm dark:text-slate-400">
                Learn Japanese verbs and their essential forms.
              </p>
            </div>
        ...
        >
          ...
          <input
            ...
            placeholder="Search verb, romaji, meaning, form..."
        ...
        {filteredVerbs.length === 0 ? (
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Try another search, level or verb group.
            </p>
      ...
      <AnimatePresence>
        {selectedVerb && (
          <VerbModal
            ...
            onLearned={() =>
              toggleLearned(selectedVerb.id)
            }
            onSpeak={() =>
              speakJapanese(selectedVerb.verb)
            }
          />
        )}
      </AnimatePresence>
